...
class ViTSTR(VisionTransformer):
    ...
    def forward_features(self, x):
        ...
        for idx, blk in enumerate(self.blocks):
            if idx == len(self.blocks) -1 :
                norm = blk.norm1
                attention = blk.attn
                transformer_input_expanded = attention.qkv(norm(x.clone()))[0]
                qkv = transformer_input_expanded.reshape(197, 3, 16, 48)  # (N=197, (qkv), H=16, D/H=64)
    ...
    def forward(self, x, seqlen=25):
        x, att_map = self.forward_features(x)
        x = x[:, :seqlen]
